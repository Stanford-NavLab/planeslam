...
if __name__ == "__main__":
    ...
    # ax7 = fig.add_subplot(2, 3, 7, projection='3d')
    # ax8 = fig.add_subplot(2, 3, 8, projection='3d')
    ...
    for i in range(len(scans)-1):
        ...
        ax4.scatter(PCs[i+1][:,0], PCs[i+1][:,1], PCs[i+1][:,2], marker='.', s=1)  
        ...
        for j, c in enumerate(clusters[i]):
            ...
            ax2.scatter3D(cluster_pts[:,0], cluster_pts[:,1], cluster_pts[:,2], color='C'+str(j), marker='.', s=1)
        for j, c in enumerate(clusters[i+1]):
            cluster_pts = mesh_cluster_pts(meshes[i+1], c)
            ax5.scatter3D(cluster_pts[:,0], cluster_pts[:,1], cluster_pts[:,2], color='C'+str(j), marker='.', s=1)
        ax2.set_box_aspect((np.ptp(PCs[i][:,0]), np.ptp(PCs[i][:,1]), np.ptp(PCs[i][:,2])))
        ...
        color_legend(ax6, len(scans[i+1].planes))
